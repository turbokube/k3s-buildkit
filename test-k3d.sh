...
# you _can_ build without tag but then the image will only be available by digest
IMAGE=example.net/g5y/test-build-in-k3d:$TESTPOD
BUILDKIT_HOST=tcp://0.0.0.0:8547 buildctl build \
...
DIGEST=$(cat /tmp/testbuild.json | grep 'containerimage.config.digest' | cut -d '"' -f4)
[ -n "$DIGEST" ] || (echo "Failed to get digest from build output" && cat /tmp/testbuild.json && cat /tmp/testbuild.json | grep 'containerimage.config.digest' && exit 1)
echo "$TESTPOD run digest $DIGEST"
...
set -x
# here the tag seems to be present
docker exec -i $SERVER ctr containers info $CONTAINER | grep '"Image"'
...
KUBECONFIG=$(pwd)/.kubeconfig kubectl run $TESTPOD-3 --image=example.net/g5y/test-build-in-k3d:$TESTPOD@$DIGEST --image-pull-policy=IfNotPresent --restart=Never
